...
def get_beautiful_embed(title, description, color=EMBED_COLOR):
    ...
    embed.set_footer(text=f"Powered by {BOT_OWNER_NAME} • ImGunpoint Cloud", icon_url=LOGO_URL)
    return embed
...
async def get_access_links(cid, os_type):
    ...
    shell_binary = "sh" if os_type == "alpine" else "bash"
    ...
    try:
        ...
        try:
            if os_type == "alpine":
                repair_cmd = "apk update && apk add curl wget tmate procps iproute2 socat openssh && curl -sSf https://sshx.io/get | sh"
            else:
                repair_cmd = "DEBIAN_FRONTEND=noninteractive apt-get update -y && DEBIAN_FRONTEND=noninteractive apt-get install -y curl wget tmate procps iproute2 socat openssh-client && curl -sSf https://sshx.io/get | sh"
...
async def deploy(interaction: discord.Interaction, user: discord.User, os_type: str, tier: str):
    ...
    try:
        ...
        setup_cmd = "DEBIAN_FRONTEND=noninteractive apt-get update -y && DEBIAN_FRONTEND=noninteractive apt-get install -y curl wget tmate procps iproute2 socat openssh-client && curl -sSf https://sshx.io/get | sh"
        shell_binary = "bash"
        if os_type == "alpine": 
            setup_cmd = "apk add curl wget tmate procps iproute2 socat openssh && curl -sSf https://sshx.io/get | sh"
            shell_binary = "sh"
            
        await run_cmd_async(["docker", "exec", cid, shell_binary, "-c", setup_cmd])
...
async def shell(interaction: discord.Interaction, cmd: str, container_id: str = None):
    ...
    await interaction.response.defer()
    try:
        shell_binary = "sh" if data.get("os") == "alpine" else "bash"
...
async def rebuild(interaction: discord.Interaction, container_id: str = None):
    ...
    try:
        ...
        await run_cmd_async(["docker", "exec", new_cid, shell_binary, "-c", setup_cmd])
